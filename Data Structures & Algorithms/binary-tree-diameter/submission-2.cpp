...
class Solution {
    unordered_map<TreeNode*,int>computedHeight;
    int findHeight(TreeNode* root) {
        if(!root) return 0;
        if(computedHeight.count(root)) return computedHeight[root];
        return computedHeight[root]=1+max(findHeight(root->left), findHeight(root->right));
    }
public:
